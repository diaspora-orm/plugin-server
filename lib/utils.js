...
var Utils = {
	...
	respondError( res, error, status ) {
		...
		jsonError.message = _.isError( error ) ? error.message || error.toString() : undefined;
		if ( error instanceof Diaspora.components.Errors.ValidationError ) {
			Diaspora.logger.debug( `Request ${ res.req.diasporaApi.id } triggered a validation error: message is ${ JSON.stringify( jsonError.message ) }`, jsonError );
			return res.status( status || 400 ).send( jsonError );
		} else {
			Diaspora.logger.error( `Request ${ _.get( res, 'req.diasporaApi.id', 'UNKNOWN' ) } triggered an error: message is ${ JSON.stringify( jsonError.message ) }`, jsonError );
