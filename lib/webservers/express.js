    ...
    optionsHandler(req, res) {
        return res.json({
            ...
            routes: _.mapValues(this._modelsConfiguration, apiDesc => ({
                [`/${apiDesc.singular}/$ID`]: this.generateApiMap(apiDesc, utils_1.EQueryPlurality.SINGULAR, req.baseUrl),
                [`/${apiDesc.plural}`]: this.generateApiMap(apiDesc, utils_1.EQueryPlurality.PLURAL, req.baseUrl),
            })),
        });
    }
    /**
