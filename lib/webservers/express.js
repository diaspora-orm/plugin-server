...
const handlers = {
	_get( model, req, res ) {
		return parseQueryError( true, false, req, res, ( query, options ) => {
			return model.find( query, options ).then( _.partial( respondMaybeNoEntity, res )).catch( _.partial( respondError, res ));
		});
	},
	_post( model, req, res ) {
	...
	get( model, req, res ) {
		return parseQueryError( false, false, req, res, ( query, options ) => {
			return model.findMany( query, options ).then( _.partial( respondMaybeEmptySet, res )).catch( _.partial( respondError, res ));
		});
	},
	post( model, req, res ) {
