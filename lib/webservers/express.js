...
const bind = ( newRouter, type, route, model, middlewares ) => {
	...
	const forbidden = (req, res) => res.status( 405 ).send({
		message: `${ req.method } is not allowed`,
	});

	if(middlewares.all === false){
		_.defaults(middlewares, {find: forbidden, update: forbidden, delete: forbidden, insert: forbidden});
		delete middlewares.all;
	}
	middlewares = _.mapValues(middlewares, value => value === false ? forbidden : value);
	middlewares = _.pickBy(middlewares, _.isFunction);
			...
			if ( 'singular' === req.diasporaApi.number ) {
				...
				if ( !_.isNil( id )) {
					if ( 'insert' === req.diasporaApi.action ) {
						return respondError( res, new Error( 'POST (insert) to explicit ID is forbidden' ), 405 );
					} else {
...
module.exports = configHash => {
	...
	newRouter.options(_.partial( handlers._, configuredModels ) );
	return newRouter;
};
